...
/*
* Se llama a la api para recibir el listado
* de un solo elemento
*/
...
if (isset($_POST['inputCI'])){    
    $client = new Client([
        'base_uri' => DIRECCION_BASE . '/api/players/'.$_POST['inputID'],
        'timeout'  => 5.0,
    ]);
    ...
    ';
    // Enviamos al servidor api la solicitud de insertar un pais
    $res = $client->request('PUT', '', ['headers' => $headers, 'body' => $info]);
    if ($res->getStatusCode() == '200') //Verifico que me retorne 200 = OK
    {
        $message = "Se actualizo un post (player)";
...
<h1>Player</h1>
  ...
  <div class="form-group">
    <h4>Country</h4>    
    <p><?php echo $data['country']['name']; ?></p>
